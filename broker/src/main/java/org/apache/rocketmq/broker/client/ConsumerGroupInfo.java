    ...
    public boolean updateSubscription(final Set<SubscriptionData> subList) {
        ...
        for (SubscriptionData sub : subList) {
            ...
            if (old == null) {
                ...
                //当消费者端的 mq 信息发生变化时 会更改版本 并重新注册到broker上
        ...
        while (it.hasNext()) {
            ...
            //查看容器中的数据是否有在  给与的set之外的  有就是就数据 这个数据就要移除
            //因为消费者组 针对一个topic 是使用同样的订阅消息的 所以某个消费者 不再订阅 某个topic了 其他消费者也要同步
            boolean exist = false;
            for (SubscriptionData sub : subList) {
            ...
            //不存在 也就是 容器中的数据在 给与的参数中找不到匹配的 就移除
            if (!exist) {
