...
public class DefaultRequestProcessor implements NettyRequestProcessor {
    ...
    public RemotingCommand processRequest(ChannelHandlerContext ctx,
        ...
        switch (request.getCode()) {
            ...
            case RequestCode.DELETE_KV_CONFIG:
                ...
                //处理 查询版本的请求 判定 该broker 是否需要将配置更新到 nsv上
            case RequestCode.QUERY_DATA_VERSION:
                return queryBrokerTopicConfig(ctx, request);
                //nsv上 处理 broker 的更新 或者注册
            case RequestCode.REGISTER_BROKER:
                //判定 版本配置 这个一般都没有设置过 就是默认值
                Version brokerVersion = MQVersion.value2Version(request.getVersion());
                //高于某个版本才支持 将brokerAddress 替换成其他地址
    ...
    public RemotingCommand registerBrokerWithFilterServer(ChannelHandlerContext ctx, RemotingCommand request)
        ...
        //在 nsv 的路由管理器上 注册对应的broker 这样 clientInstance 每次 获取最新信息的时候就能获取对应数据
        //从请求头 获取 集群名 broker 名 和地址等信息 只有当 更新的broker 是 slave 时 才会设置result 并且是 最新的 master的地址和 ha地址
    ...
    public RemotingCommand getRouteInfoByTopic(ChannelHandlerContext ctx,
        ...
        if (topicRouteData != null) {
            //如果是顺序消息 默认为 false 会存入一个 OrderTopicConfig
